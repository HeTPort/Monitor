...
- The checked-in device agent is a fixed Python implementation driven by a per-run JSON manifest. It requires `python3` on the board; a native agent remains the preferred release form when production images do not expose Python.
...
| `--baudrate INTEGER` | UART baud. | `115200`. |
...
```powershell
vmin_judge.exe --pc-serial COM8 --baudrate 115200 monitor --save-raw
```
...
/data/local/tmp/avs/bin/avs-device-agent \
  --manifest /data/local/tmp/avs/runs/RUN_ID/run-manifest.json \
  --uart /dev/ttyAMA0 \
  --baudrate 115200
```
...
| Parameter | Meaning |
|---|---|
| `--manifest PATH` | Fully resolved, hash-verified run manifest. |
| `--uart PATH` | Device UART. |
| `--baudrate N` | Informational/config validation; actual UART setup is platform-dependent. |
| `--spool-dir PATH` | Device artifact directory. |
| `--dry-run` | Validate and report actions without mutating or launching. |
...
- Keep `--per-batch-log` and `--per-frame-log` disabled on a 115200-baud UART unless a dedicated bandwidth test proves safety.
