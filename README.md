...
baseline 只在需要已批准性能阈值时通过 `--baseline` 显式启用；批准前的持续资格化用互斥的 `--golden` 只校验 checksum/readback。Telemetry 可以通过 `telemetry run` 独立采样，或在核心测试中用 `run --telemetry` 伴随启动；采样日志只追加到设备本地。所有设备证据按 `test_id/attempt_id` 保留，之后用 `collect --test-id ...` 一次性拉取。

Kirin9030 的资格化使用独立的 `cpu_qualification_kirin9030` / `gpu_qualification_kirin9030` profile。`golden --runs N` 要么不传 `--run-dir` 并从已准备设备实时生成 N 次正确性结果，要么恰好提供 N 个完整 correctness run；`source_runs` 只用于 golden 审计/合并，不能把 `duration_s=0`、`batch_count=1` 的生成运行当作性能样本。生成一致性 golden 后，用 `deploy/verify --golden` 和 `run --golden <MANIFEST> --telemetry` 收集完整持续样本，再交给离线 `calibrate`。live capture 中 `--qualification-id` 就是设备 `test_id`，每次重试另建唯一 `attempt_id`。资格化从 workload JSON 的 `timeout` 派生设备 workload guard 和生成结果前的 heartbeat 窗口，并对 summary 后等待 FINAL 使用单独的短上限；普通 `run` 的 45 秒 heartbeat 语义不变。两板、两样本、`--min-accepted 2` 只用于功能数据链验收，生产策略仍要求至少 20 个接受样本和至少 2 块板。
